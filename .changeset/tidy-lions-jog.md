...
`JSON.parse`). Added `--output <file>` to every check command and to `audit` to
write a JSON or plain-text report for CI. Removed the `extract` command
