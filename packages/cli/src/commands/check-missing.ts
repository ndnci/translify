...
export function registerCheckMissingCommand(program: Command, logger: CliLogger): void {
  program
    ...
    .addHelpText(
      ...
      `
${c.dim('Examples:')}
  ${c.brand('$')} translify check-missing
  ${c.brand('$')} translify check-missing --verbose
...
function makeAction(program: Command, logger: CliLogger) {
  return async (opts: CheckMissingOptions) => {
    ...
    try {
      ...
      if (opts.output) writeReport(opts.output, { missingKeys: missing });
      ...
      logger.spacer();
      logger.info(`Run ${c.brand('translify add-missing')} to add the missing keys.`);
      logger.spacer();
